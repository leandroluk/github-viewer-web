...
export namespace ISignUpForm {
  export type Schema = Pick<IUser, 'email' | 'password'> & { _github: Pick<IGithubUser, 'login'> };
}
...
export const SignUpForm: FC<ISignUpForm> = ({ testID, isLoading, onSubmit }) => {
  const {
    handleSubmit,
    register,
    formState: { errors },
  ...
  return (
    ...
    >
      <VStack gap={4}>
        ...
        <FormControl
          colorScheme="eco"
          isInvalid={!!errors._github?.login}
        >
          ...
          <SimpleInput
            ...
            autoComplete="off"
          />
          <FormErrorMessage>{errors._github?.login?.message as string}</FormErrorMessage>
        </FormControl>
