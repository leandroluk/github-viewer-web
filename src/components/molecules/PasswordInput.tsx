import { IconButton, InputGroup, InputRightElement, forwardRef } from '@chakra-ui/react';
...
export const PasswordInput = forwardRef<IPasswordInput, 'input'>(
  ({ onToggleVisible, variant, size, colorScheme, pr, ...restPasswordInputProps }, ref) => {
    ...
    return (
      ...
      >
        <SimpleInput
          ...
          variant={variant}
          pr={`calc(${rightRef.current?.clientWidth ?? 0}px + var(--input-padding))`}
          ref={ref}
          size={size}
          type={isVisible ? 'text' : 'password'}
        />
        <InputRightElement
          width={`${rightRef.current?.clientWidth}px`}
          mr={1}
        >
          <IconButton
            colorScheme={colorScheme}
